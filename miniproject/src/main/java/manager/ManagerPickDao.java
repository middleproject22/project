...
public class ManagerPickDao {
	...
	public void insert(ManagerPickVo vo) {
		...
		String sql = "insert into managerpick values(?,?,?)";
		try {
	...
	public ArrayList<ManagerPickVo> selectAll() {
		...
		try {
			...
			while(rs.next()) {
				list.add(new ManagerPickVo(rs.getInt(1),rs.getString(2),rs.getString(3)));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public ManagerPickVo selectByNum(int seqnum) {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			if(rs.next()) {
				vo = new ManagerPickVo(rs.getInt(1),rs.getString(2),rs.getString(3));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
